...
exports.getUserMdw = asyncHandler(async (req, res, next) => {
    try {
        ...
        // Nếu tìm thấy user
        if (user !== null) {
            // Gán user vào res.user và chuyển tiếp
            res.user = user;
            next();
        } else {
            return res.status(404).json({ success: false, message: 'Cannot find user ' + req.params.id });
        }
    } catch (err) {
        return res.status(500).json({ success: false, message: 'Error finding user ' + req.params.id + ': ' + err });
    }
});
